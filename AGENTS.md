...
- **Output-Trimmen (immer geltend):** Kommandozeilen-Output IMMER auf das Relevante reduziert in den Kontext ziehen — nie rohe Vollausgaben. Muster: (1) Logs/Errors via PowerShell `Select-String "error|Complete"` filtern statt `Get-Content`-Dump; (2) git-Output kurzformen (`git --no-pager log --oneline -5`, `git diff --stat`, bei Diffs nur `-U10`-Kontext); (3) große Dateien nicht ganz lesen, sondern per `read`-Tool `offset/limit` nur die benötigten Fenster; (4) Tool-Ergebnisse zusammenfassen, nie Roh-JSON/HTML in den Chat kopieren. Diese Reduktions-Muster sind IMMER anzuwenden — ein separater Skill dafür ist nicht nötig (Skills werden nicht garantiert geladen), die Regel gilt projektweit in dieser Datei.
