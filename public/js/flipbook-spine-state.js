...
(function () {
  ...
  var hasLoggedFlipStart = false, hasLoggedTransit = false, hasLoggedArrived = false;
  var lastFlipDirection = null, lastFlipMethod = null;

  // Sync local anchorPage with FB.anchorPage (set by flipbook-main.js on flip completion)
    ...
    try {
      ...
      if ((state === 'flipping' || state === 'user_fold' || state === 'fold_corner') && calc) {
        ...
        if (flipFromPage === null) {
          ...
          hasLoggedFlipStart = hasLoggedTransit = hasLoggedArrived = false;
          ...
          if (!hasLoggedFlipStart) {
            hasLoggedFlipStart = true;
            var iL = isForward ? (flipFromPage - 1) : Math.max(0, flipFromPage - 1);
            var iR = isForward ? (FB.totalPages - flipToPage) : (FB.totalPages - flipFromPage - 1);
            logBox('FLIP START [' + lastFlipDirection + '] [' + lastFlipMethod + ']', [
              'Page: ' + FB.currentPageNum + ' → ' + flipToPage,
              'LEFT: ' + iL + ' pg → ' + FB.linesFor(iL) + ' strips',
              'RIGHT: ' + iR + ' pg → ' + FB.linesFor(iR) + ' strips'
            ]);
          }
        }
        ...
        var cL, cR, phase = lin < 0.95 ? 'TRANSIT' : 'ARRIVED';
        if (lin < 0.95) { cL = FB.linesFor(fL); cR = FB.linesFor(fR); }
        else { cL = FB.linesFor(fL + (isForward ? 1 : 0)); cR = FB.linesFor(fR); }

        if (!hasLoggedTransit && phase === 'TRANSIT') {
          hasLoggedTransit = true;
          console.log('┌────────────────────────────────────────────────────┐');
          console.log('│ TRANSIT [' + lastFlipDirection + '] ' + Math.round(lin*100) + '% │');
          console.log('│ L:' + cL + ' (' + fL + 'pg)  R:' + cR + ' (' + fR + 'pg) │');
          console.log('└────────────────────────────────────────────────────┘');
        }
        if (!hasLoggedArrived && phase === 'ARRIVED') {
          hasLoggedArrived = true;
          console.log('┌────────────────────────────────────────────────────┐');
          console.log('│ ARRIVED [' + lastFlipDirection + '] │');
          console.log('│ L:' + cL + ' strips  R:' + cR + ' strips │');
          console.log('└────────────────────────────────────────────────────┘');
        }
        ...
        if (flipFromPage !== null) {
          var sL = FB.linesFor(FB.currentPageNum-1), sR = FB.linesFor(FB.totalPages-FB.currentPageNum);
          logBox('REST STATE', ['Page '+FB.currentPageNum, 'L:'+sL+' R:'+sR]);
        }
